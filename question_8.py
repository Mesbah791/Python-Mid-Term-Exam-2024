            ...
            for seat in seats_to_book:
                ...
                if 1 <= row <= self.rows and 1 <= col <= self.cols:
                    if self.seats[show_id][row - 1][col - 1] == 'Free':
                        ...
                        print(f"Seat {row}{chr(64 + col)} is already booked.") #Error
                else:
                    print(f"Invalid seat {row}{chr(64 + col)} for show ID {show_id}.") #Errpr
        else:
            print(f"Show with ID {show_id} does not exist.") #Error
